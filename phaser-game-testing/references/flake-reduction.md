...
2. **Capture evidence on failure:**
   - Console messages (via `agent-browser errors` or `agent-browser console`)
   - Network requests with status codes (via `agent-browser network requests`)
   - Screenshot at failure moment (via `agent-browser screenshot failure.png`)
   - State dump from `window.__TEST__.state()` (via `agent-browser eval "window.__TEST__.state()"`)
...
```bash
# In test via agent-browser
agent-browser wait --fn "window.__TEST__?.ready === true"
# Or
agent-browser eval "window.__TEST__?.ready === true"
```
